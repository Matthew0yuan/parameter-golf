"""SP8192 recurrent-gated Parameter Golf experiment.

Readable development version of the 2026-04-15 record-track script.
It keeps the SOTA SP8192 recurrence/TTT/GPTQ stack and adds:
- depth-conditioned scalar gates on recurrent residual lanes
- W_down-only random-basis adapters on recurrent layers
- deterministic mixed-bit GPTQ allocation attempts
"""
...
class Hyperparameters:
    ...
    depth_gates_enabled = bool(int(os.environ.get('DEPTH_GATES_ENABLED', '1')))
    depth_gate_max_delta = float(os.environ.get('DEPTH_GATE_MAX_DELTA', 0.25))
    wdown_adapter_rank = int(os.environ.get('WDOWN_ADAPTER_RANK', 4))
    wdown_adapter_alpha = float(os.environ.get('WDOWN_ADAPTER_ALPHA', 0.1))
    wdown_adapter_seed = int(os.environ.get('WDOWN_ADAPTER_SEED', 4242))
    mixed_bits_enabled = bool(int(os.environ.get('MIXED_BITS_ENABLED', '1')))
    mixed_artifact_target_bytes = int(os.environ.get('MIXED_ARTIFACT_TARGET_BYTES', 15980000))
...
class RandomBasisWDownAdapter(nn.Module):

    def __init__(self, hidden, dim, rank, alpha, seed, layer_idx):
        super().__init__()
        self.alpha = alpha
        g = torch.Generator(device='cpu')
        g.manual_seed(int(seed) + 1009 * int(layer_idx))
        self.register_buffer('basis_in', torch.randn(hidden, rank, generator=g, dtype=torch.float32) / math.sqrt(max(hidden, 1)), persistent=False)
        self.register_buffer('basis_out', torch.randn(rank, dim, generator=g, dtype=torch.float32) / math.sqrt(max(rank, 1)), persistent=False)
        self.scale = nn.Parameter(torch.zeros(rank, dtype=torch.float32))

    def forward(self, x):
        z = torch.matmul(x, self.basis_in.to(device=x.device, dtype=x.dtype))
        z = z * self.scale.to(dtype=x.dtype)
        return self.alpha * torch.matmul(z, self.basis_out.to(device=x.device, dtype=x.dtype))

class MLP(nn.Module):

    def __init__(self, dim, mlp_mult, layer_idx=-1, h=None):
        super().__init__()
        hidden = int(mlp_mult * dim)
        self.fc = CastedLinear(dim, hidden, bias=False)
        self.proj = CastedLinear(hidden, dim, bias=False)
        self.proj._zero_init = True
        enabled = h is not None and h.wdown_adapter_rank > 0 and (h.loop_start <= layer_idx <= h.loop_end)
        self.adapter = RandomBasisWDownAdapter(hidden, dim, h.wdown_adapter_rank, h.wdown_adapter_alpha, h.wdown_adapter_seed, layer_idx) if enabled else None

    def forward(self, x):
        h = F.leaky_relu(self.fc(x), negative_slope=0.5).square()
        y = self.proj(h)
        return y + self.adapter(h) if self.adapter is not None else y

class Block(nn.Module):

    def __init__(self, dim, num_heads, num_kv_heads, mlp_mult, rope_base, qk_gain_init, train_seq_len, layer_idx=0, ln_scale=False, h=None):
        super().__init__()
        self.attn_norm = RMSNorm()
        self.mlp_norm = RMSNorm()
        self.attn = CausalSelfAttention(dim, num_heads, num_kv_heads, rope_base, qk_gain_init, train_seq_len)
        self.mlp = MLP(dim, mlp_mult, layer_idx, h)
    ...
    def forward(self, x, x0, depth_gate=None):
        ...
        if self.parallel:
            ...
            x_out = (
                x_in
                + attn_gate * self.attn_scale.to(dtype=x_in.dtype)[None, None, :] * attn_out
                + mlp_gate * self.mlp_scale.to(dtype=x_in.dtype)[None, None, :] * mlp_out
            )
        else:
            x_out = x_in + attn_gate * self.attn_scale.to(dtype=x_in.dtype)[None, None, :] * attn_out
            x_out = x_out + mlp_gate * self.mlp_scale.to(dtype=x_out.dtype)[None, None, :] * self.mlp(self.mlp_norm(x_out) * self.ln_scale_factor)
        return x_out
...
class GPT(nn.Module):
    ...
    def __init__(self, h):
        ...
        self.depth_gates_enabled = h.depth_gates_enabled and h.num_loops > 0
        self.depth_gate_max_delta = h.depth_gate_max_delta
        num_recurrent_calls = sum((1 for idx in self.encoder_indices + self.decoder_indices if self.loop_start <= idx <= self.loop_end))
        self.depth_gates = nn.Parameter(torch.zeros(num_recurrent_calls, 2, dtype=torch.float32)) if self.depth_gates_enabled else None
    ...
    def _depth_gate_for(self, layer_idx, gate_pos):
        if self.looping_active and self.depth_gates is not None and (self.loop_start <= layer_idx <= self.loop_end):
            raw = self.depth_gates[gate_pos]
            return (1.0 + self.depth_gate_max_delta * torch.tanh(raw), gate_pos + 1)
        return (None, gate_pos)
    ...
    def forward_logits(self, input_ids):
        ...
        for i in enc_iter:
            depth_gate, gate_pos = self._depth_gate_for(i, gate_pos)
            x = self.blocks[i](x, x0, depth_gate)
            skips.append(x)
...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        'q_gain,skip_weight,skip_weights,skip_gates,depth_gates',
    ).split(',')
    if pattern
)
...
class Optimizers:
    ...
    def __init__(self, h, base_model):
        ...
        if base_model.depth_gates is not None and base_model.depth_gates.numel() > 0:
            scalar_params.append(base_model.depth_gates)
...
def _hessian_importance(name, hessians):
    H = hessians.get(name)
    if H is None:
        return float('inf')
    return float(torch.diag(H).float().mean().item())

def _late_int5_candidates(state_dict, hessians, h):
    c = [
        name
        for name, t in state_dict.items()
        if name.endswith('mlp.fc.weight')
        and _block_idx(name) >= h.parallel_residual_start
        and name in hessians
        and t.is_floating_point()
        and t.numel() > 65536
    ]
    c.sort(key=lambda n: (-_block_idx(n), _hessian_importance(n, hessians)))
    return c

def _is_recurrent_writeback(name, h):
    b = _block_idx(name)
    return h.loop_start <= b <= h.loop_end and (name.endswith('attn.proj.weight') or name.endswith('mlp.proj.weight'))

def _quant_policy(name, h, int5_names, drop_attn_int7):
    if 'tok_emb' in name:
        return (h.embed_bits, h.embed_clip_sigmas, 'embed')
    if name in int5_names:
        return (5, h.matrix_clip_sigmas, 'mixed:int5_late_mlp_fc')
    if h.mixed_bits_enabled and _is_recurrent_writeback(name, h):
        if drop_attn_int7 and name.endswith('attn.proj.weight'):
            return (h.matrix_bits, h.matrix_clip_sigmas, 'mixed:int6_recurrent_attn_budget')
        return (7, h.matrix_clip_sigmas, 'mixed:int7_recurrent_writeback')
    return (h.matrix_bits, h.matrix_clip_sigmas, 'matrix')

def gptq_mixed_quantize(state_dict, hessians, h, int5_names=None, drop_attn_int7=False):
    int5_names = set() if int5_names is None else set(int5_names)
    result = {}
    meta = {}
    for name, tensor in state_dict.items():
        t = tensor.detach().cpu().contiguous()
        if not t.is_floating_point() or t.numel() <= 65536 or name not in hessians:
            result[name] = t.to(torch.float16) if t.is_floating_point() else t
            meta[name] = 'passthrough (float16)'
            continue
        bits, cs, reason = _quant_policy(name, h, int5_names, drop_attn_int7)
        q, s = gptq_quantize_weight(t, hessians[name], clip_sigmas=cs, clip_range=2 ** (bits - 1) - 1)
        result[name + '.q'] = q
        result[name + '.scale'] = s
        meta[name] = f'gptq int{bits} clip={cs:g} {reason}'
...
def serialize(h, base_model, code):
    ...
    late_candidates = _late_int5_candidates(sd_cpu, hessians, h) if h.mixed_bits_enabled else []
    attempts = [(set(), False, 'base')]
    if h.mixed_bits_enabled:
        attempts = [
            (set(), False, 'recurrent-int7'),
            (set(late_candidates), False, 'recurrent-int7+late-int5'),
            (set(late_candidates), True, 'late-int5-drop-attn-int7'),
        ]
    last = None
    for int5_names, drop_attn_int7, attempt_name in attempts:
        log(f'mixed_bit_attempt:{attempt_name} target_bytes:{h.mixed_artifact_target_bytes} int5:{len(int5_names)} drop_attn_int7:{drop_attn_int7}')
        quant_result, quant_meta = gptq_mixed_quantize(sd_cpu, hessians, h, int5_names=int5_names, drop_attn_int7=drop_attn_int7)
        ...
        last = (quant_blob, quant_file_bytes, bytes_total)
        log(f'mixed_bit_attempt_result:{attempt_name} model_bytes:{quant_file_bytes} total_bytes:{bytes_total}')
        if not h.mixed_bits_enabled or bytes_total <= h.mixed_artifact_target_bytes:
            break
    quant_blob, quant_file_bytes, bytes_total = last
    if h.mixed_bits_enabled and bytes_total > h.mixed_artifact_target_bytes:
        log(f'mixed_bit_budget_warning:total_bytes:{bytes_total} target:{h.mixed_artifact_target_bytes}')
    if h.is_main_process:
